...
export function normalizeAiWayReasoningLevel(value='off'){
  const v=String(value||'off').toLowerCase();
  return ['off','low','medium','high','xhigh'].includes(v)?v:'off';
}
function reasoningLevel(payload={}){return normalizeAiWayReasoningLevel(payload.aiway_reasoning_level)}

/**
 * Resolve the UI's four reasoning choices to the actual controls supported by
 * a provider/model family. Keep this policy centralized so adding a model does
 * not require changing the composer or every transport adapter.
 */
export function reasoningPolicy(provider='',model='',level='off'){
  const uiLevel=normalizeAiWayReasoningLevel(level),p=String(provider||'').toLowerCase(),id=String(model||'').toLowerCase();
  const base={provider:p,model:id,uiLevel,family:'generic',enabled:uiLevel!=='off',effort:uiLevel==='off'?null:uiLevel};

  // DeepSeek V4 Chat Completions: low/high/max. medium and xhigh are merely
  // compatibility aliases upstream, so map intentionally to real effort tiers.
  if(/deepseek-v4-(?:flash|flash-vision-exp)/.test(id)){
    return {...base,family:'deepseek-v4-flash',enabled:uiLevel!=='off',effort:uiLevel==='off'?null:({low:'low',medium:'low',high:'high',xhigh:'max'})[uiLevel]||'low',thinkingType:uiLevel==='off'?'disabled':'enabled'};
  }
  if(/deepseek-v4-pro/.test(id)){
    return {...base,family:'deepseek-v4-pro',enabled:uiLevel!=='off',effort:uiLevel==='off'?null:({low:'high',medium:'high',high:'high',xhigh:'max'})[uiLevel]||'high',thinkingType:uiLevel==='off'?'disabled':'enabled'};
  }

  // OpenAI reasoning models expose none/low/medium/high/xhigh. The AiWay UI
  // intentionally has no separate low option, so Medium maps to medium.
  if(/^gpt-5(?:[.-]|$)|^o[134](?:[.-]|$)/.test(id)){
    return {...base,family:'openai-reasoning',enabled:uiLevel!=='off',effort:uiLevel==='off'?'none':uiLevel};
  }

  // Hy3 has a provider-specific explicit no-thinking token.
  if(id==='hy3'||id==='hy3-free'||id.endsWith('/hy3')||id.endsWith('/hy3-free')){
    return {...base,family:'hy3',enabled:uiLevel!=='off',effort:uiLevel==='off'?'no_think':uiLevel};
  }

  // OpenRouter normalizes effort itself. Preserve its supported generic tiers.
  if(p==='openrouter') return {...base,family:'openrouter',effort:uiLevel==='off'?null:uiLevel};

  return base;
}

export function applyBaiReasoningPayload(payload={}){
  const out=structuredClone(payload),policy=reasoningPolicy('bai',out.model,out.aiway_reasoning_level);
  delete out.aiway_reasoning_level;
  if(policy.family==='deepseek-v4-flash'||policy.family==='deepseek-v4-pro'){
    out.thinking={type:policy.thinkingType};
    if(policy.enabled) out.reasoning_effort=policy.effort;
    else delete out.reasoning_effort;
    // DeepSeek ignores sampling parameters in thinking mode; removing them
    // avoids misleading settings and keeps payload semantics explicit.
    if(policy.enabled){delete out.temperature;delete out.top_p;delete out.presence_penalty;delete out.frequency_penalty;}
    return out;
  }
  if(policy.family==='openai-reasoning'){
    out.reasoning_effort=policy.effort;
    return out;
  }
  // Unknown B.ai model families are left untouched rather than sending a
  // reasoning field they may reject. This is the safe forward-compatible path.
  return out;
}
...
function responseReasoningEffort(model,level){const policy=reasoningPolicy('opencode',model,level);if(policy.family==='openai-reasoning')return policy.effort;if(level!=='off')return level;const id=String(model||'').toLowerCase();return /^gpt-5(?:$|-(?:mini|nano)$)/.test(id)?'minimal':'none'}
...
function hy3ReasoningEffort(level){return reasoningPolicy('opencode','hy3',level).effort}

export function sanitizeOpenAIChatPayload(payload={}){
  const level=reasoningLevel(payload),policy=reasoningPolicy('opencode',payload.model,level);
  const out={...payload};
  delete out.aiway_reasoning_level;
  ...
  } else if(policy.family==='deepseek-v4-flash'||policy.family==='deepseek-v4-pro'){
    out.thinking={type:policy.thinkingType};
    if(policy.enabled){out.reasoning_effort=policy.effort;delete out.temperature;delete out.top_p;delete out.presence_penalty;delete out.frequency_penalty;}
    else delete out.reasoning_effort;
  } else if(level!=='off') out.reasoning_effort=policy.effort||level;
...
export async function proxyHermesRun({payload,sessionId,signal},res){
  ...
  const rp=reasoningPolicy(provider||'hermes',model,reasoningLevel(payload));
  const create=await fetch(`${base}/v1/runs`,{method:'POST',headers,body:JSON.stringify({input,instructions,conversation_history:history,session_id:sessionId||undefined,model,provider,model_options:{reasoning_effort:rp.effort||(rp.enabled?'high':'none')}}),signal});
