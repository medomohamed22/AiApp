 ...
 *
 * Common response, auth, rate-limit, request parsing, cancellation, streaming, and OpenCode helpers. Keep reusable server logic here or in other /lib modules—not /api.
 *
...
export async function pipeFetch(upstream, res, signal) {
  ...
  // Preserve only non-secret rate-limit hints so the browser can pace agent turns
  // according to the upstream provider instead of blindly retrying a 429.
  for (const header of ['retry-after','x-ratelimit-limit-requests','x-ratelimit-remaining-requests','x-ratelimit-reset-requests','x-ratelimit-limit-tokens','x-ratelimit-remaining-tokens','x-ratelimit-reset-tokens']) {
