...
must(js.includes('taskCap=state.settings.provider==="bai"?1:8'), 'provider-aware delegate_task cap missing');
must(js.includes('batchSize=state.settings.provider==="bai"?1:4'), 'provider-aware subagent batch size guard missing');
must(js.includes('providerRatePolicy')&&js.includes('reserveProviderRequest')&&js.includes('rpm:8')&&js.includes('provider429DelayMs'), 'B.ai request pacing guard missing');
