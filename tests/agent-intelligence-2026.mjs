...
must(js.includes('taskCap=state.settings.provider==="bai"?2:8'), 'provider-aware delegate_task cap missing');
must(js.includes('batchSize=state.settings.provider==="bai"?2:4'), 'provider-aware subagent batch size guard missing');
must(js.includes('providerRatePolicy')&&js.includes('reserveProviderRequest')&&js.includes('rpm:9'), 'B.ai request pacing guard missing');
