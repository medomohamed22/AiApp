...
const nativeDefs={
 ...
 artifact_read:{description:"Read a bounded slice of one artifact by id or exact name. Prefer project_search first, then read only the exact range needed. Large files stay lossless in storage and can be continued with nextOffset.",parameters:{type:"object",properties:{id:{type:"string"},name:{type:"string"},offset:{type:"number",description:"Zero-based character offset; default 0"},maxChars:{type:"number",description:"Characters to return in this call; default 16000, max 32000. Request a larger slice only when exact surrounding context is required."}}}},
...
async function runAgent(chat,userText){
  ...
  let round=0,visionInjected=0;const skillCache=new Map(),effectiveMaxRounds=Math.min(24,Math.max(4,+state.settings.maxRounds||12));
