...
async function runAgent(chat,userText){
  ...
  while(round++<effectiveMaxRounds){
    ...
    contents.push({role:"user",parts:responseParts});afterToolRound(turn.toolCalls,executed);if(consecutiveReadRounds>=2)contents.push({role:"user",parts:[{text:"[RUNTIME EFFICIENCY] Multiple evidence-reading rounds are complete. Unless a specific missing fact blocks the task, apply the edit now, verify locally, or finalize; do not start another read/search loop."}]});
