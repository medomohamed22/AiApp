...
function providerRatePolicy(provider=""){
 ...
 if(p==="bai")return{rpm:8,windowMs:60000,label:"B.ai"};
...
async function reserveProviderRequest(provider=""){
 ...
 const task=async()=>{
   while(true){
     const now=Date.now(),cooldown=Math.max(0,(providerCooldownUntil.get(provider)||0)-now);
     if(cooldown>0){
       pushRunActivity("rate_guard","يعمل…",`المزود طلب تهدئة مؤقتة — هنكمل تلقائيًا بعد ${Math.ceil(cooldown/1000)}ث`,`تنظيم الطلبات`);
       setActivity("thinking",`ينتظر تهدئة ${policy.label} بدل تكرار طلبات 429`);
       await sleep(cooldown+80);continue;
     }
     const times=(providerRequestTimes.get(provider)||[]).filter(t=>now-t<policy.windowMs);
     ...
     const wait=Math.max(250,times[0]+policy.windowMs-now+180);
...
function provider429DelayMs(response,provider="",attempt=0){
 const explicit=retryAfterMs(response);if(explicit>0)return explicit;
 const policy=providerRatePolicy(provider),now=Date.now(),times=(providerRequestTimes.get(provider)||[]).filter(t=>now-t<(policy.windowMs||60000));
 const localWindow=times.length?Math.max(0,times[0]+(policy.windowMs||60000)-now+250):0;
 const fallback=String(provider||"").toLowerCase()==="bai"?30000:Math.min(15000,1000*(2**attempt));
 return Math.min(120000,Math.max(localWindow,fallback)+Math.floor(Math.random()*900));
}
...
const nativeDefs={
 ...
 artifact_read:{description:"Read one artifact by id or exact name. For a single known file, omit maxChars to read the whole file in one call when it is <=220k characters. For larger files or narrow inspection, pass offset/maxChars. Do not reread ranges already returned.",parameters:{type:"object",properties:{id:{type:"string"},name:{type:"string"},offset:{type:"number",description:"Zero-based character offset; default 0"},maxChars:{type:"number",description:"Optional slice size. If omitted, AiWay returns the entire file when <=220k chars; explicit reads are capped at 220k chars."}}}},
 ...
 artifact_edit:{description:"Safely patch one existing artifact without resending the whole file. The runtime first tries an exact match, then safe line-ending and whitespace-normalized recovery when the target is uniquely identifiable. It verifies the persisted result and reports structured failure reasons instead of silently returning changed:false. You can also submit multiple edits in one atomic call with edits:[{oldText,newText,replaceAll?}] to reduce model/tool rounds. Prefer this over artifact_save for targeted changes to large files.",parameters:{type:"object",properties:{name:{type:"string",description:"Exact artifact name"},oldText:{type:"string",description:"Literal text to replace for a single edit."},newText:{type:"string",description:"Replacement text for a single edit. Empty string deletes the target."},replaceAll:{type:"boolean",description:"Replace every exact/uniquely-normalized occurrence for a single edit."},edits:{type:"array",maxItems:24,items:{type:"object",properties:{oldText:{type:"string"},newText:{type:"string"},replaceAll:{type:"boolean"}},required:["oldText","newText"]},description:"Optional atomic batch of targeted edits. If provided, oldText/newText at the top level are ignored."}},required:["name"]}},
...
function nativeToolRouteScore(name,plan,hasProjectFiles=false){
 ...
 add(name==='artifact_read'&&hasProjectFiles&&s.artifactRead,92);add(name==='project_search'&&hasProjectFiles&&s.artifactRead,88);add(name==='artifact_list'&&hasProjectFiles&&s.artifactRead,18);
...
async function artifactEdit(args){
 ...
 if(!hit)return{ok:false,changed:false,reason:"artifact_not_found",error:`Artifact not found: ${args.name}. Use project_search or artifact_read to confirm the exact name.`};
 const original=String(hit.content||"");
 const requested=Array.isArray(args.edits)&&args.edits.length?args.edits.slice(0,24):[{oldText:args.oldText,newText:args.newText,replaceAll:args.replaceAll}];
 if(!requested.length)return{ok:false,changed:false,reason:"no_edits",error:"No edits were supplied."};
 const normalizeEol=x=>String(x??"").replace(/\r\n?/g,"\n");
 const normalizeLoose=x=>normalizeEol(x).split("\n").map(line=>line.trim().replace(/[ \t]+/g," ")).join("\n").trim();
 const indexAll=(hay,needle)=>{const out=[];if(!needle)return out;let i=0;while((i=hay.indexOf(needle,i))!==-1){out.push(i);i+=Math.max(1,needle.length)}return out};
 const findNormalizedRange=(content,target)=>{
  const wanted=normalizeLoose(target);if(!wanted)return{reason:"empty_target"};
  const lines=normalizeEol(content).split("\n"),targetLines=normalizeEol(target).split("\n");
  const count=Math.max(1,targetLines.length),matches=[];let offset=0;const starts=[];
  for(const line of lines){starts.push(offset);offset+=line.length+1}
  for(let i=0;i<=lines.length-count;i++){
   const candidate=lines.slice(i,i+count).join("\n");
   if(normalizeLoose(candidate)===wanted){const start=starts[i],end=i+count<starts.length?starts[i+count]-1:normalizeEol(content).length;matches.push({start,end,text:normalizeEol(content).slice(start,end)})}
  }
  if(matches.length===1)return{...matches[0],mode:"whitespace-normalized"};
  if(matches.length>1)return{reason:"ambiguous_normalized",matches:matches.length};
  return{reason:"target_not_found"};
 };
 let working=original,totalReplacements=0,recovered=0,alreadyApplied=0;const details=[];
 for(let n=0;n<requested.length;n++){
  const edit=requested[n]||{},oldText=String(edit.oldText??""),newText=String(edit.newText??""),replaceAll=edit.replaceAll===true;
  if(!oldText)return{ok:false,changed:false,reason:"empty_target",failedEdit:n+1,error:`Edit ${n+1}: oldText must not be empty.`};
  if(oldText===newText){alreadyApplied++;details.push({edit:n+1,status:"noop",reason:"replacement_identical"});continue}
  let matches=indexAll(working,oldText),mode="exact",updated=working,replacements=0;
  if(matches.length){
   if(matches.length>1&&!replaceAll)return{ok:false,changed:false,reason:"ambiguous_match",failedEdit:n+1,matches:matches.length,error:`Edit ${n+1}: oldText matched ${matches.length} times. Include more surrounding context or pass replaceAll:true.`};
   updated=replaceAll?working.split(oldText).join(newText):working.replace(oldText,newText);replacements=replaceAll?matches.length:1;
  }else{
   // If the replacement is already present, this is a verified idempotent success,
   // not a mysterious changed:false failure that makes the agent give up.
   const newMatches=newText?indexAll(working,newText):[];
   if(newText&&newMatches.length===1){alreadyApplied++;details.push({edit:n+1,status:"already_applied",reason:"replacement_present"});continue}
   const normalized=findNormalizedRange(working,oldText);
   if(normalized.start!=null){
    mode=normalized.mode;updated=working.slice(0,normalized.start)+newText+working.slice(normalized.end);replacements=1;recovered++;
   }else{
    return{ok:false,changed:false,reason:normalized.reason||"target_not_found",failedEdit:n+1,matches:normalized.matches||0,error:`Edit ${n+1}: target was not found safely. Re-read only the relevant region and retry with a larger unique anchor.`,recovery:{recommended:"artifact_read",instruction:"Read the smallest relevant region, then retry artifact_edit once with exact surrounding context. Do not abandon the task solely because this edit missed."}};
   }
  }
  if(updated===working){alreadyApplied++;details.push({edit:n+1,status:"already_applied",reason:"result_identical",mode});continue}
  working=updated;totalReplacements+=replacements;details.push({edit:n+1,status:"applied",mode,replacements});
 }
 if(working===original){return{ok:true,changed:false,alreadyApplied:true,reason:"already_applied",name:hit.name,verified:true,details,note:"All requested edits were already present or were no-ops; the file is already in the requested state."};}
 // One transactional write for the entire batch. saveArtifactRecord keeps the normal version snapshot.
 const obj=await saveArtifactRecord({id:hit.id,name:hit.name,language:hit.language,content:working});
 const verify=await findProjectArtifact(hit.name),persisted=String(verify?.content||"");
 if(!verify||persisted!==working){return{ok:false,changed:false,reason:"verification_failed",error:"The edit was computed but persistence verification failed. The runtime will not claim the file was updated.",details};}
 await renderArtifacts();
 return{ok:true,name:obj.name,changed:true,verified:true,replacements:totalReplacements,recoveredEdits:recovered,alreadyAppliedEdits:alreadyApplied,editCount:requested.length,charsBefore:original.length,charsAfter:working.length,versioned:true,details};
}
...
function finalizationInstruction(reason="tools"){const why=reason==="max_rounds"?"انتهت نافذة التنفيذ الحالية بعد عدد كبير من خطوات الأدوات.":"اكتملت الأدوات لكن لم يتم إرسال رد نصي.";return `${why} اكتب الآن أفضل رد نهائي اعتمادًا على النتائج الفعلية. لا تستدعِ أي أداة أخرى في هذه الجولة. لا تذكر حدود الجولات أو الطلبات أو تفاصيل الـruntime للمستخدم، ولا تقل إنك تحتاج وضعًا آخر أو أدوات نشر إذا كانت الأدوات قد كانت متاحة بالفعل. إذا كان المستخدم طلب تعديل ملف ولم يتم حفظ التعديل فعليًا، اذكر فقط العائق الخارجي الحقيقي إن وُجد؛ وإلا لخّص ما تم حفظه والتحقق منه.`}
...
- When project/file retrieval is explicitly needed, retrieve only the smallest relevant snippets or, for one known moderate file, one full read.  Use one narrow project_search first; when it identifies a single file <=220k characters, read that file once in full, then edit only the requested regions. For larger files, use exact ranges. Avoid rereading the same range, batch independent reads in one turn, and do not scan every file unless the user explicitly asks for a full audit.
...
async function runAgent(chat,userText){
  ...
  if(hasAttachedArtifact){for(const name of ["project_search","artifact_read"]){if((state.toolPermissions[name]||"auto")!=="off"&&!defs.some(x=>x.name===name)){const d=nativeDefs[name];defs.push({name,description:d.description,parameters:d.parameters,source:"native",permission:state.toolPermissions[name]||"auto",deferred:false,routeScore:100})}}}
  ...
  system+=`\n\nMODEL-OWNED TOOL ROUTING (OpenAI-style):\n- tool_choice is AUTO. You decide whether any tool is needed; answer directly when no tool can improve the answer.\n- A tiny core catalog is loaded initially: web_search (when enabled) plus tool_search. The long-tail catalog remains deferred.\n- Use web_search directly whenever the answer depends on live/current/external facts, recent news, leaks/rumors, current product status, prices, schedules, leadership, software versions, or verification that may have changed.\n- Never say that live web access is unavailable when web_search is present in your tools. If current information is required, call it before answering.\n- If project files, execution, memory, Skills, MCP, publishing, verification, or another capability may materially help and the needed tool is not loaded, call tool_search with the capability you need.\n- tool_search dynamically loads only a small relevant subset of executable tools. After it returns, choose among those tools yourself. You may call tool_search again for a different capability later in the same run.\n- Minimize model turns: when several independent reads/searches are known, call them together in the same turn instead of one per turn.\n- For project work, use project_search to identify the exact file(s). If one known file is <=220k characters, read it once in full with artifact_read (omit maxChars), then patch only the requested parts. For larger files, read only the necessary ranges. Never reread the same range or walk the repository file-by-file.\n- Prefer one evidence-gathering batch followed by one edit batch and one local verification batch. Deterministic validators do not justify extra model turns unless they find an error that needs a fix. Stop as soon as the requested change is persisted and verified.\n- When the user explicitly asks you to modify an attached/project file, completing the actual artifact_edit/artifact_save is the goal. Do not stop after analysis merely because several tool rounds were used. Continue until the requested file change is persisted and then verify it, unless an external permission/error truly blocks the write.\n- artifact_edit is recoverable: changed:false with alreadyApplied:true means success; a target_not_found/ambiguous result means re-read only the smallest relevant region and retry ONCE with a larger unique anchor. Prefer one atomic edits[] batch for multiple changes. Never tell the user that the editing tool "refused" or that you must wait for another session while artifact_read/artifact_edit/artifact_save remain available.\n- If a targeted edit still cannot match after one fresh read, use artifact_save only when you have a complete current copy of that file; preserve every untouched byte/section and then verify. Never regenerate an unread large file from memory.\n- Do not mention internal round/request/tool limits to the user. The runtime paces provider calls and may wait automatically; treat that as an implementation detail.\n- On B.ai, avoid delegate_task for ordinary coding/file edits because each subagent consumes another model request; prefer local search/read/edit tools in the main agent.\n- Never infer that a tool ran merely because it exists; use returned evidence before claiming success.\n- For a relevant Skill, tool_search returns Skill names and loads skill_read; read only the Skill you need.`;
  ...
  let round=0,visionInjected=0,consecutiveReadRounds=0,writeSucceeded=false,recoverableEditFailure=false,recoveryRoundsAdded=0;const editIntent=/\b(edit|modify|change|fix|update|patch|rewrite|save)\b|عدل|تعديل|اصلح|إصلاح|غير|غيّر|اكتب|احفظ/i.test(String(userText||""));const skillCache=new Map(),callFingerprints=new Map(),budgetPlan=runtimeAdaptiveBudget||adaptiveTaskBudget(userText,chat);let effectiveMaxRounds=budgetPlan.rounds;currentRunInspector=currentRunInspector||{};currentRunInspector.adaptiveBudget=budgetPlan;if(budgetPlan.extended)pushRunActivity("smart_router","تم",`وسّع ميزانية المهمة تلقائيًا إلى ${effectiveMaxRounds} جولات و ${budgetPlan.output.toLocaleString()} output tokens`,"ميزانية ذكية");
  const runTool=async(call)=>{
    const resolved=await resolveAgentTool(defs,call),tool=resolved.tool,args=resolved.args;let result;const fp=`${tool?.name||call.name}:${JSON.stringify(args||{})}`;const seen=callFingerprints.get(fp)||0;if(seen>=1&&!["artifact_edit","artifact_save","memory_save"].includes(tool?.name)){return{result:{ok:true,skippedDuplicate:true,note:"Duplicate tool call suppressed; reuse the previous result instead of repeating the same operation."},tool,args}}callFingerprints.set(fp,seen+1);
    ...
    try{
      ...
      if(tool.name==="artifact_edit"){if(result?.ok&&(result?.changed||result?.alreadyApplied))writeSucceeded=true;recoverableEditFailure=!!(!result?.ok&&["target_not_found","ambiguous_match","ambiguous_normalized","verification_failed"].includes(result?.reason))}
      if(tool.name==="artifact_save"&&result?.saved)writeSucceeded=true;
  ...
  const READ_ONLY_ROUND_TOOLS=new Set(["tool_search","project_search","artifact_read","artifact_list","skill_read","skill_list","skill_resource_list","skill_resource_read","memory_search","session_search","web_search","browser_extract","sandbox_read","environment_list"]);
  const afterToolRound=(calls,executed)=>{const names=calls.map((c,i)=>executed[i]?.tool?.name||c.name),readOnly=names.length&&names.every(n=>READ_ONLY_ROUND_TOOLS.has(n));consecutiveReadRounds=readOnly?consecutiveReadRounds+1:0;const largeRead=executed.some(x=>(x?.result?.totalChars||0)>180000);if(largeRead&&effectiveMaxRounds<Math.min(state.settings.provider==="bai"?7:9,(budgetPlan.rounds||6)+1)){effectiveMaxRounds++;pushRunActivity("smart_router","تم",`الملف كبير؛ مدّدت التنفيذ بجولة واحدة فقط إلى ${effectiveMaxRounds}`,"ميزانية ذكية")}return{names,readOnly};};
  if(state.settings.provider!=="gemini"){
    ...
    while(round++<effectiveMaxRounds){
      ...
      const executed=await runToolBatch(turn.toolCalls);for(const [i,{result}] of executed.entries()){const call=turn.toolCalls[i];messages.push({role:"tool",tool_call_id:call.id,content:JSON.stringify(result)})}const roundState=afterToolRound(turn.toolCalls,executed);if(editIntent&&!writeSucceeded&&recoverableEditFailure&&round>=effectiveMaxRounds&&recoveryRoundsAdded<2){effectiveMaxRounds++;recoveryRoundsAdded++;messages.push({role:"user",content:"[WRITE RECOVERY] The requested file change is not yet persisted. A recoverable edit mismatch occurred. Re-read only the smallest target region, retry artifact_edit once with a unique anchor/batch, then verify. Do not finalize with an excuse while write tools remain available."});pushRunActivity("write_recovery","تم",`مدّدت التنفيذ بجولة استعادة كتابة (${recoveryRoundsAdded}/2)` ,"استعادة التعديل")}if(consecutiveReadRounds>=2)messages.push({role:"user",content:"[RUNTIME EFFICIENCY] You have already completed multiple evidence-reading rounds. Unless a specific missing fact blocks the task, do not perform another search/read round. Apply the requested edit now, run local verification, or finalize."});
  ...
  while(round++<effectiveMaxRounds){
    ...
    clearProvisionalStreamForToolCall();
    contents.push(turn.nativeAssistant);const responseParts=[],executed=await runToolBatch(turn.toolCalls);
    for(const [i,{result,tool}] of executed.entries()){const call=turn.toolCalls[i],functionResponse={name:tool?.name||call.name,response:result};if(call.id)functionResponse.id=call.id;responseParts.push({functionResponse})}
    contents.push({role:"user",parts:responseParts});afterToolRound(turn.toolCalls,executed);if(editIntent&&!writeSucceeded&&recoverableEditFailure&&round>=effectiveMaxRounds&&recoveryRoundsAdded<2){effectiveMaxRounds++;recoveryRoundsAdded++;contents.push({role:"user",parts:[{text:"[WRITE RECOVERY] The requested file change is not yet persisted. Re-read only the smallest target region, retry artifact_edit once with a unique anchor/batch, then verify. Do not finalize with an excuse while write tools remain available."}]});pushRunActivity("write_recovery","تم",`مدّدت التنفيذ بجولة استعادة كتابة (${recoveryRoundsAdded}/2)` ,"استعادة التعديل")}if(consecutiveReadRounds>=2)contents.push({role:"user",parts:[{text:"[RUNTIME EFFICIENCY] Multiple evidence-reading rounds are complete. Unless a specific missing fact blocks the task, apply the edit now, verify locally, or finalize; do not start another read/search loop."}]});
...
async function send(){
  ...
  try{
    ...
    if(controller===requestController)controller=null;runtimeModelOverride="";runtimeUserQuery="";runtimeContextPlan=null;runtimeAdaptiveBudget=null;currentRunVisionImages=[];currentSearchRoute="web";$("#sendBtn").classList.remove("stop");$("#sendBtn").textContent="➤";renderToggles();updateScrollButton();
  }
}
...
async function optimizedProjectContext(userText=""){
 ...
 const manualChars=Math.max(120000,+state.settings.contextCharBudget||360000),projectChars=Math.max(5000,Math.min(12000,Math.floor(manualChars*.16)));const found=await projectSearchExact(text,{maxResults:Math.min(10,Math.max(4,Math.floor(manualChars/25000))),maxChars:projectChars});if(!found.results.length)return`\n\n${head}`;
...
function adaptiveTaskBudget(text=runtimeUserQuery||"",chat=null){
 const t=String(text||""),limits=modelSafetyLimits(),provider=state.settings.provider;
 const coding=/(code|coding|debug|fix|html|css|javascript|typescript|python|project|file|artifact|كود|برمج|خطأ|اصلح|عدل|ملف|مشروع)/i.test(t);
 const broad=/(full|complete|entire|all files|audit|review|architecture|refactor|comprehensive|كامل|شامل|كل الملفات|راجع|معمار|ريفاكتور)/i.test(t);
 const long=t.length>1800,bigProject=!!runtimeContextPlan&&runtimeContextPlan.totalMessages>30;
 let rounds=+state.settings.maxRounds||6;if(coding)rounds=Math.max(rounds,6);if(broad||long)rounds=Math.max(rounds,7);if(broad&&coding)rounds=Math.max(rounds,8);if(bigProject)rounds=Math.max(rounds,7);
 rounds=Math.min(provider==="bai"?7:10,Math.max(3,rounds));
 let output=+state.settings.maxOutputTokens||12000;if(coding||long)output=Math.max(output,12000);if(broad&&coding)output=Math.max(output,16000);output=Math.min(output,limits.maxOutputTokens,32768);
 let activeTokens=Math.floor((+state.settings.contextCharBudget||360000)/2.9);activeTokens=Math.max(60000,activeTokens);if(coding)activeTokens=Math.max(activeTokens,110000);if(broad||long)activeTokens=Math.max(activeTokens,150000);activeTokens=Math.min(activeTokens,Math.floor(limits.contextWindow*.78));
 return{rounds,output,activeTokens,provider,extended:rounds>(+state.settings.maxRounds||6)||output>(+state.settings.maxOutputTokens||12000)};
}
function safeOutputTokens(){const limits=modelSafetyLimits(),adaptive=runtimeAdaptiveBudget||adaptiveTaskBudget(),requested=Math.max(2048,adaptive.output||+state.settings.maxOutputTokens||12000);return Math.max(128,Math.min(requested,limits.maxOutputTokens,Math.max(128,limits.contextWindow-4096)))}
...
function contextTokenPlan(chat,system="",tools=[]){
 ...
 const manualChars=Math.max(120000,+state.settings.contextCharBudget||360000),adaptive=runtimeAdaptiveBudget||adaptiveTaskBudget(),manualCap=Math.max(30000,Math.min(adaptive.activeTokens||120000,Math.floor(manualChars/2.9)));
...
function prepareOpenAIContext(messages=[],system="",tools=[],{emergency=false}={}){
 let out=structuredClone(messages||[]),plan=contextTokenPlan(null,system,tools),budget=plan.hardMessageBudget,before=requestTokenEstimateOpenAI(out,"",[]),threshold=Math.min(budget*.88,Math.max(90000,plan.inputBudget*.92)),target=Math.floor(emergency?Math.min(budget*.68,Math.max(50000,plan.inputBudget*.72)):Math.min(budget*.84,Math.max(80000,plan.inputBudget*.88))),cleared=0,compacted=0,truncated=0;if(!emergency&&before<=threshold)return{messages:out,meta:{beforeTokens:before,afterTokens:before,budget,cleared:0,compacted:0,truncated:0,emergency:false,plan,skipped:true}};
...
function prepareGeminiContext(contents=[],system="",tools=[],{emergency=false}={}){
 let out=structuredClone(contents||[]),plan=contextTokenPlan(null,system,tools),budget=plan.hardMessageBudget,before=requestTokenEstimateGemini(out,"",[]),threshold=Math.min(budget*.88,Math.max(90000,plan.inputBudget*.92)),target=Math.floor(emergency?Math.min(budget*.68,Math.max(50000,plan.inputBudget*.72)):Math.min(budget*.84,Math.max(80000,plan.inputBudget*.88))),cleared=0,compacted=0,truncated=0;if(!emergency&&before<=threshold)return{contents:out,meta:{beforeTokens:before,afterTokens:before,budget,cleared:0,compacted:0,truncated:0,emergency:false,plan,skipped:true}};
...
function noteContextGuard(meta){if(!meta||(meta.cleared+meta.compacted+meta.truncated)<=0)return;currentRunInspector=currentRunInspector||{};currentRunInspector.contextCompactions=[...(currentRunInspector.contextCompactions||[]),{time:Date.now(),beforeTokens:meta.beforeTokens,afterTokens:meta.afterTokens,budget:meta.budget,clearedToolResults:meta.cleared,compactedMessages:meta.compacted,truncatedToolResults:meta.truncated,emergency:meta.emergency}].slice(-12);if(meta.emergency)pushRunActivity("context_manager","تم",`حماية نافذة السياق: ~${meta.beforeTokens.toLocaleString()} → ~${meta.afterTokens.toLocaleString()} tokens`,"إدارة السياق")}

function selectContextMessages(chat,system="",tools=[]){
 const base=(chat?.messages||[]).filter(m=>m.role==="user"||m.role==="assistant"),plan=contextTokenPlan(chat,system,tools),limit=Math.max(2,Math.min(+state.settings.historyLimit||20,40));
