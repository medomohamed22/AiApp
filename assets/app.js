...
const nativeDefs={
 ...
 artifact_read:{description:"Read a bounded slice of one artifact by id or exact name. Large files are never silently truncated in storage: use offset/maxChars repeatedly, or project_search first, until hasMore is false.",parameters:{type:"object",properties:{id:{type:"string"},name:{type:"string"},offset:{type:"number",description:"Zero-based character offset; default 0"},maxChars:{type:"number",description:"Characters to return in this call; default 24000, max 60000"}}}},
 project_search:{description:"Search project files and return only the most relevant exact code snippets. Prefer this before reading large files to reduce context tokens without rewriting the source.",parameters:{type:"object",properties:{query:{type:"string",description:"What code, symbol, error, feature or filename to find"},maxResults:{type:"number",description:"Maximum snippets; default 6, max 12"},maxChars:{type:"number",description:"Total returned characters; default 12000, max 24000"}},required:["query"]}},
...
- When project/file retrieval is explicitly needed, retrieve only the smallest relevant snippets instead of broad file sets.
...
async function runAgent(chat,userText){
  ...
  let round=0,visionInjected=0;const skillCache=new Map();
...
function compactProjectManifest(name,entries,skipped,totalEntries){
 const paths=entries.map(x=>x.name),shown=paths.slice(0,140),more=Math.max(0,paths.length-shown.length);
...
async function projectSearchExact(query,{maxResults=6,maxChars=12000}={}){
 const terms=contextTerms(query),all=(await idbAll("artifacts")).filter(x=>x.projectId===state.settings.activeProjectId),cap=Math.max(2000,Math.min(24000,+maxChars||12000)),limit=Math.max(1,Math.min(12,+maxResults||6));
 ...
 let used=0,results=[];for(const r of ranked){if(results.length>=limit||used>=cap)break;const remain=cap-used,piece=snippetAround(r.a.content,r.first<0?0:r.first,Math.min(2600,remain));if(!piece)continue;results.push({id:r.a.id,name:r.a.name,language:r.a.language,score:r.score,snippet:piece});used+=piece.length}
...
async function optimizedProjectContext(userText=""){
 ...
 const manualChars=Math.max(8000,+state.settings.contextCharBudget||50000),projectChars=Math.max(8000,Math.min(24000,Math.floor(manualChars*.18)));const found=await projectSearchExact(text,{maxResults:Math.min(10,Math.max(4,Math.floor(manualChars/25000))),maxChars:projectChars});if(!found.results.length)return`\n\n${head}`;
...
function prepareOpenAIContext(messages=[],system="",tools=[],{emergency=false}={}){
 let out=structuredClone(messages||[]),plan=contextTokenPlan(null,system,tools),budget=plan.hardMessageBudget,target=Math.max(1024,Math.floor(budget*(emergency?.68:.86))),before=requestTokenEstimateOpenAI(out,"",[]),cleared=0,compacted=0,truncated=0;
 const names=toolNameByCallId(out),toolIdx=out.map((m,i)=>m?.role==="tool"?i:-1).filter(i=>i>=0),keepNewest=emergency?1:2;
...
function prepareGeminiContext(contents=[],system="",tools=[],{emergency=false}={}){
 let out=structuredClone(contents||[]),plan=contextTokenPlan(null,system,tools),budget=plan.hardMessageBudget,target=Math.max(1024,Math.floor(budget*(emergency?.68:.86))),before=requestTokenEstimateGemini(out,"",[]),cleared=0,compacted=0,truncated=0;
